...
/**
 * Use case for restoring a deleted record.
 */
class RestoreRecordUseCase @Inject constructor(
    private val recordRepository: RecordRepository
) : UseCase<RestoreRecordUseCase.Params, Unit>() {

    override suspend fun execute(parameters: Params): Unit {
        require(parameters.recordId > 0) { "Record ID must be valid" }
        
        when (val result = recordRepository.restoreRecord(parameters.recordId)) {
